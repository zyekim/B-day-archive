  ...
  return (
    <main className="relative mx-auto flex min-h-screen max-w-2xl flex-col items-center justify-center overflow-hidden px-6">
      ...
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        <Patch seed="entry-1" size={70} className="absolute left-[8%] top-[14%]" />
        <Patch seed="entry-2" size={58} className="absolute right-[10%] top-[20%]" />
        <Patch seed="entry-3" size={64} className="absolute left-[14%] bottom-[16%]" />
        <Patch seed="entry-4" size={52} className="absolute right-[14%] bottom-[20%]" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-md text-center">
        <p className="mb-2 font-hand text-2xl text-dusty-rose">2000년대 우리들의</p>
        ...
        <form onSubmit={go} className="mx-auto w-full max-w-sm bg-polaroid p-5 shadow-polaroid">
          <label htmlFor="name" className="mb-2 block text-left font-pixel text-[11px] text-album-navy">
            친구 이름을 입력하세요
          </label>
          <input
            id="name"
